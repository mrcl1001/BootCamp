...
object JdbcProvisioner {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      statement.execute("CREATE TABLE bytes(timestamp TIMESTAMP, id TEXT, value BIGINT, tipo TEXT)")

      println("Creando la tabla bytes para modelar las métricas agregadas de bytes")
      statement.execute("CREATE TABLE bytes_hourly(timestamp TIMESTAMP, id TEXT, value BIGINT, tipo TEXT)")
      println("Creando la tabla bytes para modelar las métricas agregadas de bytes")
